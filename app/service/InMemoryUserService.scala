 ...
 */
class InMemoryUserService extends UserService[User.User] {
  val logger = Logger("application.controllers.InMemoryUserService")

  var users = Map[(String, String), User.User]()
  ...
  private def updateProfile(user: BasicProfile, entry: ((String, String), User.User)): Future[User.User] = {
  ...
  def save(user: BasicProfile, mode: SaveMode): Future[User.User] = {
    mode match {
      case SaveMode.SignUp =>
        val newUser = User.User(user, List(user))
  ...
  def link(current: User.User, to: BasicProfile): Future[User.User] = {
  ...
  override def updatePasswordInfo(user: User.User, info: PasswordInfo): Future[Option[BasicProfile]] = {
    Future.successful {
      for (
  ...
  override def passwordInfoFor(user: User.User): Future[Option[PasswordInfo]] = {
    Future.successful {
      for (
